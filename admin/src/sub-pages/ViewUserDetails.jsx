...
const ViewUserDetails = () => {
  ...
  // Fetch product details function (not needed but kept for reference)
  const fetchProductDetails = async (productId) => {
    try {
      ...
      return { name: "Unnamed Product", imageUrls: [""] };
    }
  };
    ...
    const fetchUserDetails = async () => {
      try {
        ...
        if (cart && cart.items && cart.items.length > 0) {
          // Use the populated productId data directly from the response
  ...
  const calculateTotalPrice = () => {
    return cartItemsWithDetails.reduce((total, item) => total + (item.price || 0) * item.quantity, 0);
  };
              ...
              {cartItemsWithDetails.length > 0 ? (
                cartItemsWithDetails.map((item, index) => (
                  <tr key={item._id || index} className="border-b">
                    ...
                    <td className="p-3">{item.productDetails.name || "Unnamed Product"}</td>
