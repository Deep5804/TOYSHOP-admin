...
const Products = () => {
  ...
  // Extract category from query parameters (subcategory not in your API data)
  const searchParams = new URLSearchParams(location.search);
  const selectedCategory = searchParams.get("category") || "";
  ...
  const filteredProducts = products.filter((product) => {
    const matchesCategory =
      !selectedCategory ||
      product.productType.includes(selectedCategory); // Using productType as category
    const matchesSearch =
      product.name.toLowerCase().includes(search.toLowerCase()) ||
      product.description.toLowerCase().includes(search.toLowerCase()) ||
      product.materialType.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="p-6 bg-gray-100">
      {/* Header */}
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-semibold text-gray-800">Toy Products</h1>
      ...
      {filteredProducts.length === 0 && (
        <p className="text-gray-500 w-full mt-5 text-center">No products found.</p>
      )}
      <div
        className={
          viewMode === "grid"
