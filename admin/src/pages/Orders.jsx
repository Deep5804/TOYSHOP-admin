...
const Orders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await axios.get(
          "https://toyshop-sooty.vercel.app/api/orders",
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const matchesSearch =
      ...
      order.products.some((product) =>
        product.productId.name.toLowerCase().includes(searchLower)
      );
  ...
  const handleSaveStatus = async () => {
    try {
      setIsLoading(true);
      const response = await axios.put(
        `https://toyshop-sooty.vercel.app/api/orders/${selectedOrder._id}`,
        {
